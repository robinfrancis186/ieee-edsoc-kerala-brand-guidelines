    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      >
        ...
        >
          <div>
            <img
              src={textColor === "#fff" ? "/ieee-logo-white.png" : "/ieee-logo-colour.png"}
              alt="IEEE Logo"
              style={{ height: 36 }}
            />
          </div>
...
export default function PosterPatterns() {
  return (
    <div
      style={{
        minHeight: "100vh",
    ...
    >
      ...
      <section style={{ marginBottom: 52 }}>
        ...
        >
          {[
            ...
            >
              <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 24, minHeight: 120 }}>
                {item.el}
              </div>
              <div style={{ padding: "8px 12px", background: "rgba(255,255,255,0.9)", borderTop: "1px solid rgba(0,0,0,0.05)" }}>
                <p style={{ fontSize: 10, fontWeight: 600, color: "#3D4A5C", margin: 0, textAlign: "center" }}>{item.label}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* ── 5. Full Poster Mockups ── */}
      <section>
        <SectionHeader title="Full Poster Mockups" />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 24 }}>
          <PosterMockup
            label="Tech Seminars (Navy Primary)"
            bg={NAVY}
            accent={ORANGE}
            patternEl={<PatternDots id="pm-1" color={NAVY_LIGHT} bg={NAVY} />}
          />
          <PosterMockup
            label="Workshops (Dark Mode)"
            bg={NAVY_DARK}
            accent={ORANGE_LIGHT}
            patternEl={<PatternCircuits id="pm-2" color={NAVY_LIGHT} bg={NAVY_DARK} />}
          />
          <PosterMockup
            label="Light / Minimal (White)"
            bg="#fff"
            accent={ORANGE}
            textColor={NAVY}
            patternEl={<PatternDiagonalLines id="pm-3" color={NAVY} bg="#fff" />}
          />
          <PosterMockup
            label="Special Events (Orange)"
            bg={ORANGE}
            accent={NAVY_DARK}
            textColor={NAVY_DARK}
            patternEl={<PatternWaves id="pm-4" color={ORANGE_DARK} bg={ORANGE} />}
          />
        </div>
      </section>
    </div>
